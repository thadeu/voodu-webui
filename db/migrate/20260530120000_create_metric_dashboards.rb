...
# One server has many; a single one may be pinned (the default view
...
class CreateMetricDashboards < ActiveRecord::Migration[8.1]
  def change
    ...
    add_index :metric_dashboards, [:server_id, :name], unique: true

    # At most one pinned dashboard per server. The model's pin! also
    # enforces this transactionally; the partial unique index is the
    # DB-level backstop against a concurrent double-pin.
    add_index :metric_dashboards, :server_id, unique: true,
      where: "pinned = 1",
      name: "index_metric_dashboards_one_pinned_per_server"
  end
end
