...
Both the Ruby shim and the Go binary check `LOG_POLLER_SPAWN`. If it is not
exactly `"1"`, they exit 0 immediately so Puma will not restart-storm.

| Env var                          | Default                     | Notes                              |
| -------------------------------- | --------------------------- | ---------------------------------- |
| `LOG_POLLER_SPAWN`                 | unset (disabled)            | Set to `1` to enable               |
| `LOG_POLLER_TOKEN`      | required                    | Auth to Rails internal endpoint    |
| `RAILS_INTERNAL_URL`             | `http://127.0.0.1:3000`     | Rails app base URL                 |
| `LOG_POLLER_INTERVAL_SECONDS`    | `15` (min `5`)              | Per-island poll cadence            |
| `LOG_POLLER_STORAGE_DIR`         | `./storage/logs`            | NDJSON output root                 |
| `LOG_POLLER_OBSERVABILITY_ADDR`  | `:9999`                     | Bind for `/healthz` + `/metrics`   |
| `LOG_POLLER_VERBOSE`             | unset (silent)              | Set to `1` to log one summary line per island per tick (pods/scanned/written/deduped/elapsed/wm_lag) |

## Building the binary

```sh
cd gems/log_poller && make build
```

Produces `gems/log_poller/src/log-poller`. The Puma plugin and the Ruby
binstub both resolve to that path via `LogPoller.binary_path`.
...
```sh
LOG_POLLER_SPAWN=1 LOG_POLLER_TOKEN=... bin/log-poller
```

## Production / Docker

The repo's `Dockerfile` builds the binary in a dedicated multi-stage
`log-poller-build` stage (alpine + Go toolchain, isolated from the
Ruby image) and `COPY --from=log-poller-build` lands the artifact at
`gems/log_poller/src/log-poller` in the final image. No Go toolchain
ships in the runtime image. The binary inherits the container's env
vars, so set `LOG_POLLER_TOKEN` via kamal secrets / docker
...
If `LOG_POLLER_SPAWN=1`, the plugin spawns the binary on `on_booted`, sends
