...
type IslandPoller struct {
	...
	// regardless. Wired from `LOG_POLLER_VERBOSE=1` in main.go.
	Verbose bool
...
func (p *IslandPoller) Run(ctx context.Context) {
	...
	if err != nil {
		log.Printf("[log_poller] %s: lock failed (%v) — skipping this run", p.Island.ID, err)
...
func (p *IslandPoller) tick(ctx context.Context) {
	...
	if err != nil {
		...
		log.Printf("[log_poller] %s: fetch failed: %v", p.Island.ID, err)
	...
	for scanner.Scan() {
		...
		if err := p.Writer.Append(p.Island.ID, pod, rec); err != nil {
			log.Printf("[log_poller] %s/%s: append failed: %v", p.Island.ID, pod, err)
			p.Metrics.ErrorIncr(p.Island.ID)
	...
	if err := scanner.Err(); err != nil && !errors.Is(err, io.EOF) {
		log.Printf("[log_poller] %s: scan error: %v", p.Island.ID, err)
		p.Metrics.ErrorIncr(p.Island.ID)
	}
	...
	for pod, ts := range latest {
		if err := WriteWatermark(p.Root, p.Island.ID, pod, ts); err != nil {
			log.Printf("[log_poller] %s/%s: watermark write failed: %v", p.Island.ID, pod, err)
			p.Metrics.ErrorIncr(p.Island.ID)
		}
	}
	...
	// LOG_POLLER_VERBOSE=1. Skipped otherwise so a healthy poller
	// stays silent (errors + lifecycle still log either way).
	if p.Verbose {
		log.Printf(
			"[log_poller] tick island=%s pods=%d scanned=%d written=%d deduped=%d elapsed=%s wm_lag=%s",
