...
//
//   - The Rails app (`/internal/log_poller/islands`) for the list of
...
// Island is one row from /internal/log_poller/islands. Mirrors the
...
// FetchIslands GETs `/internal/log_poller/islands` and returns the
// parsed island list. Refuses an unknown `version`.
func (c *RailsClient) FetchIslands() ([]Island, error) {
	req, err := http.NewRequest("GET", c.BaseURL+"/internal/log_poller/islands", nil)
	if err != nil {
