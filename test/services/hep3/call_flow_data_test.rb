...
class Hep3::CallFlowDataTest < ActiveSupport::TestCase
  ...
  # A real Server, not a Struct double: HepMessage's readers take a Server and
  # refuse anything else, because these rows carry a bare server_id with no org
  # column (see ServerScoped). A double that answers #id would sail past the
  # one check standing between a caller and another tenant's SIP capture.
