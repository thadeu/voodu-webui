...
module Internal
  class PollerControllerTest < ActionDispatch::IntegrationTest
    ...
    test "returns 200 + server roster with right token from loopback" do
      get internal_poller_servers_path,
      ...
      assert_kind_of Array, body["servers"]
      assert_operator body["servers"].length, :>=, 2

      sample = body["servers"].find { |i| i["key"] == "aaaaaa" }
      assert sample, "expected fixture server alpha in response"
