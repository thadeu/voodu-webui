...
module Internal
  class LogPollerControllerTest < ActionDispatch::IntegrationTest
    fixtures :islands
    ...
    setup do
      ...
      ENV["LOG_POLLER_TOKEN"] = INTERNAL_TOKEN
    end

    teardown do
      ENV.delete("LOG_POLLER_TOKEN")
    end

    test "returns 401 without X-Voodu-Internal-Token header" do
      get internal_log_poller_islands_path
