...
# /islands is the TENANT-LESS landing (no org/tenant in the URL), but its
# sidebar + server list build per-server links (`/:org_id/:tenant_key/…`).
...
class IslandsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index renders the server list with org-scoped per-server links" do
    ...
    alpha = islands(:alpha)
    # The sidebar + list link each server at /:org_id/:tenant_key — the org
