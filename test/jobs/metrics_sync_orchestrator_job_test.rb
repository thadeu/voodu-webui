...
class MetricsSyncOrchestratorJobTest < ActiveJob::TestCase
  ...
  test "early-returns without touching Server when POLLER_SPAWN=1" do
    ...
    begin
      assert_no_enqueued_jobs do
        MetricsSyncOrchestratorJob.new.perform
      end
  ...
  test "runs normal fan-out when POLLER_SPAWN is unset" do
    ...
    assert_enqueued_jobs Server.count, only: MetricsSyncServerJob do
      MetricsSyncOrchestratorJob.new.perform
    end
  end
end
