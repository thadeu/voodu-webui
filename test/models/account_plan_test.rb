...
#
# The installation license answers "what is this box"; a plan answers "what did
...
class AccountPlanTest < ActiveSupport::TestCase
  ...
  # The fixture keypair the rest of the license tests use. Signing with the real
  ...
  # An installation license: signed by us, valid, and about the BOX. It names a
  ...
  # ── A license that names no plan is not a plan license ─────────────────
  #
  ...
  test "an installation license pasted into the plan form is refused" do
    with_test_key do
      status, detail = @account.activate_plan!(installation_token)

      assert_equal :not_a_plan, status
      assert_equal "enterprise", detail, "the tier is what tells them which license they pasted"
    end
  end

  test "a refused installation license is not stored" do
    with_test_key do
  ...
  # installation license would not make it a plan license, so "it expired" would
  # send somebody to buy the wrong thing again.
  test "an expired installation license is refused for what it is, not for its date" do
    with_test_key do
      expired = JWT.encode({
  ...
  # A plan claim the build does not recognize is refused rather than read as
  ...
  # warns that it will stop, so a license stored that way works now and dies on
  ...
  test "an account with no license is on the free plan" do
    assert_equal "free", @account.plan
    assert_not @account.pro?
  end

  test "activating a plan license issued for it moves it to pro" do
  ...
  # The binding. Without it a pro license is a file that circulates by email.
  test "a license issued for another account is refused" do
  ...
  test "a foreign license already in the column grants nothing" do
  ...
  # Grace applies to a plan the same way it applies to the box's license: a
  ...
  test "one customer's purchase does not move another's limits" do
    other_owner = User.create!(email: "neighbor@example.com", email_verified: true)
    Account.provision!(owner: other_owner, account_name: "Globex", org_name: "Prod")
    neighbor = other_owner.owned_accounts.first
    ...
    assert_not Entitlements.for(neighbor, HOSTED).room_for_another_org?,
      "the neighbor's allowance moved when somebody else bought a plan"
  end
  ...
  # Self-hosted is untouched: there the box's license governs and an account
  ...
  test "a self-hosted installation ignores account plans entirely" do
    ...
    # question that actually matters: on a self-hosted box the LICENSE decides
    # the plan, so the account's own token is never consulted.
    assert_equal "pro", entitlements.plan, "the box's Enterprise license upgrades its one account"
