...
# StateSyncOrchestratorJob — fans out per-server state syncs.
#
# Recurs every 10 seconds (see config/recurring.yml). Each tick:
#
#   1. Enumerate every registered Server.
#   2. perform_later one StateSyncServerJob per server, so each
#      server's HTTP fetch + snapshot replacement runs in parallel
#      against solid_queue's worker pool.
#
# Sibling of `MetricsSyncOrchestratorJob` — same fan-out pattern,
# same rationale (per-server isolation, retry granularity, worker
...
# (every 14s).
#
# Empty-server case (greenfield WebUI before any server is added)
...
class StateSyncOrchestratorJob < ApplicationJob
  ...
  def perform
    ...
    Server.find_each do |server|
      StateSyncServerJob.perform_later(server.id)
    end
  end
end
