...
class HepCursor < HepRecord
  ...
  def self.cursor_for(server, scope, name)
    find_by(server_id: server_id_of(server), scope: scope, name: name)&.cursor.to_s
  end

  # advance — persist the next cursor for this instance (upsert on the
  # unique (server, scope, name) index).
  def self.advance(server, scope, name, cursor)
    rec = find_or_initialize_by(server_id: server_id_of(server), scope: scope, name: name)
