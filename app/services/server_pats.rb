...
# ServerPats — single source of truth for "give me the PAT list of
# this server".
#
# Distinct from ServerPods / ServerSystem because the failure mode
...
class ServerPats
  ...
  def self.fetch(client, server)
    return Result.new(ok: false, error: "no server") if client.nil? || server.nil?

    cached = Rails.cache.read(cache_key(server))
    return Result.new(ok: true, pats: cached) if cached.is_a?(Array)

    pats = client.pats
    Rails.cache.write(cache_key(server), pats, expires_in: TTL)
    Result.new(ok: true, pats: pats)
  rescue Voodu::Client::AuthError => e
    Rails.logger.info("server_pats: PAT lacks admin scope: #{e.message}")
    ...
    Rails.logger.warn("server_pats: transport #{e.message}")
    Result.new(ok: false, error: "Controller unreachable — tokens will load once it's back online.")
  rescue Voodu::Client::Error => e
    Rails.logger.warn("server_pats: #{e.class} #{e.message}")
  ...
  def self.cache_key(server)
    "voodu:pats:v1:server:#{server.id}"
  end
end
